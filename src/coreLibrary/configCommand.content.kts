        ...
        addSub(CommandInfo(thisRef, "config", "查看或修改配置") {
            ...
            body {
                ...
                if (arg[0].equals("reload", true)) {
                    ...
                    return@body reply("[green]重载成功".with())
                }
                val config = arg.firstOrNull()?.let { ConfigBuilder.all[it] } ?: return@body reply("[red]找不到配置项".with())
                when (arg.getOrNull(1)?.toLowerCase()) {
                    ...
                    "set" -> {
                        if (arg.size <= 2) return@body reply("[red]请输入值".with())
