...
command("mInfo", "获取用户信息", {
    ...
    if (arg.isEmpty()) return@command reply("[red]请输入玩家uuid".with())
    val player = netServer.admins.getInfo(arg[0]) ?: return@command reply("[red]玩家未找到".with())

    @Suppress("EXPERIMENTAL_API_USAGE")
    val data = transaction { PlayerData.find(player) }
            ?: return@command reply("[red]玩家未找到".with())
